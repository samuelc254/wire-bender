...
int main(void) {
	...
	while (HAL_GPIO_ReadPin(end_stop_GPIO_Port, end_stop_Pin)) {
		HAL_GPIO_WritePin(motor_a_GPIO_Port, motor_a_Pin, HIGH);
	}

	encoder_position = 0;
	HAL_GPIO_WritePin(motor_a_GPIO_Port, motor_a_Pin, LOW);
...
void bend(int angle) {
	if (angle > encoder_position) {
		HAL_GPIO_WritePin(motor_a_GPIO_Port, motor_a_Pin, LOW);
		HAL_GPIO_WritePin(motor_b_GPIO_Port, motor_b_Pin, HIGH);
	} else {
		HAL_GPIO_WritePin(motor_a_GPIO_Port, motor_a_Pin, HIGH);
		HAL_GPIO_WritePin(motor_b_GPIO_Port, motor_b_Pin, LOW);
	}
	while (encoder_position != angle) {
...
void feed(int amount) {
	if (amount < 0)
		HAL_GPIO_WritePin(dir_GPIO_Port, dir_Pin, HIGH);
	else
		HAL_GPIO_WritePin(dir_GPIO_Port, dir_Pin, LOW);
